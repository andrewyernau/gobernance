---
tags:
  - rust
  - rust/book
  - rust/mapa
status: activo
---

# Rust Book - Hub

Esta es la nota raiz de `learning/book`. Su funcion no es explicar un tema concreto, sino darte una entrada estable al vault.

## Empieza aqui

- [[Rust Book - Roadmap]]
- [[USAGE]]
- [[Proyecto - Sandbox con Vulkan]]
- [[Proyecto - Cubix - Triangulo y bootstrap Vulkan]]

## Capítulos activos

- [[Capitulo 01 - Empezando]]
- [[Capitulo 02 - Juego de adivinanzas]]
- [[Capitulo 03 - Conceptos comunes]]
- [[Capitulo 04 - Ownership]]
- [[Capitulo 05 - Structs]]
- [[Capitulo 06 - Enums y match]]
- [[Capitulo 07 - Paquetes, crates y modulos]]
- [[Capitulo 08 - Colecciones comunes]]
- [[Capitulo 09 - Manejo de errores]]

## Conceptos base

- [[Concepto - Rust Toolchain]]
- [[Concepto - Cargo]]
- [[Concepto - Variables y mutabilidad]]
- [[Concepto - Tipos de datos]]
- [[Concepto - Funciones y expresiones]]
- [[Concepto - Flujo de control]]
- [[Concepto - Ownership]]
- [[Concepto - Borrowing y referencias]]
- [[Concepto - Slices]]
- [[Concepto - Structs]]
- [[Concepto - Metodos y funciones asociadas]]
- [[Concepto - Enums]]
- [[Concepto - Match]]
- [[Concepto - Result]]
- [[Concepto - Option]]
- [[Concepto - if let y let else]]
- [[Concepto - Paquetes y crates]]
- [[Concepto - Modulos]]
- [[Concepto - Paths y use]]
- [[Concepto - Vec]]
- [[Concepto - String y str]]
- [[Concepto - HashMap]]
- [[Concepto - Panic]]
- [[Concepto - Propagacion de errores]]

## Regla de uso

Si creas una nota nueva, intenta enlazarla al menos con:

- un capitulo;
- un concepto;
- el proyecto [[Proyecto - Sandbox con Vulkan]] si aplica.
